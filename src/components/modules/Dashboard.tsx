  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-[#0c0c0e] p-6 lg:p-10 overflow-y-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 shrink-0">
        ...
        <div className="flex items-center gap-4 bg-gray-50 dark:bg-[#141414] border border-gray-200 dark:border-[#262626] px-6 py-3 rounded-xl shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-10">
        ...
        <div className="col-span-1 md:col-span-2 lg:col-span-1 bg-white dark:bg-[#141414] border border-gray-200 dark:border-[#262626] p-6 rounded-xl shadow-sm flex flex-col justify-between group hover:border-gray-300 dark:hover:border-[#333] transition-colors">
        ...
        {/* Activity Heatmap */}
        <div className="col-span-1 md:col-span-2 lg:col-span-2 bg-white dark:bg-[#141414] border border-gray-200 dark:border-[#262626] p-6 rounded-xl shadow-sm group hover:border-gray-300 dark:hover:border-[#333] transition-colors">
        ...
        {/* Mini Finances */}
        <div className="col-span-1 md:col-span-1 lg:col-span-1 bg-white dark:bg-[#141414] border border-gray-200 dark:border-[#262626] p-6 rounded-xl shadow-sm flex flex-col justify-between group cursor-pointer hover:border-gray-300 dark:hover:border-[#333] transition-colors" onClick={() => onNavigate?.('Income')}>
          <div className="flex items-center gap-2 mb-4 opacity-90">
            <Wallet className="w-5 h-5 text-accent" />
            <h3 className="font-semibold text-gray-900 dark:text-white">Total Balance</h3>
          </div>
          <div>
            <p className="text-4xl font-bold tracking-tight text-gray-900 dark:text-white">{formatCurrency(balance)}</p>
            <p className="text-sm mt-2 text-gray-500 dark:text-zinc-500 flex items-center gap-1 group-hover:text-gray-900 dark:group-hover:text-zinc-300 transition-colors">View ledger <ChevronRight className="w-3 h-3" /></p>
          </div>
        </div>

        {/* Mini Tasks */}
        <div className="col-span-1 md:col-span-2 bg-white dark:bg-[#141414] border border-gray-200 dark:border-[#262626] p-6 rounded-xl shadow-sm flex flex-col group hover:border-gray-300 dark:hover:border-[#333] transition-colors cursor-pointer" onClick={() => onNavigate?.('Tasks')}>
        ...
        {/* Mini Notes */}
        <div className="col-span-1 md:col-span-2 bg-white dark:bg-[#141414] border border-gray-200 dark:border-[#262626] p-6 rounded-xl shadow-sm flex flex-col group hover:border-gray-300 dark:hover:border-[#333] transition-colors cursor-pointer" onClick={() => onNavigate?.('Notes')}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
          ...
          <div className="flex-1 space-y-2">
            {(!vaultData?.notes || vaultData.notes.length === 0) ? (
              ...
              vaultData.notes.slice().sort((a, b) => b.updatedAt - a.updatedAt).slice(0, 3).map(n => (
                <div key={n.id} className="flex items-center gap-3 text-sm bg-gray-50 dark:bg-[#141414] p-2 rounded-md">
...
const ActionButton = ({ icon: Icon, label, onClick }: { icon: any, label: string, onClick: () => void }) => (
  <button 
    onClick={onClick}
    className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-[#141414] border border-gray-200 dark:border-[#262626] hover:bg-gray-50 dark:hover:bg-[#1e1e1e] rounded-lg text-sm font-medium text-gray-700 dark:text-zinc-300 transition-colors shadow-sm"
  >
    <Icon size={16} />
    {label}
  </button>
