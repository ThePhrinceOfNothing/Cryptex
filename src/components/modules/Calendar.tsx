        ...
        {upcoming.map(item => {
          ...
          return (
            <React.Fragment key={item.id}>
              {showHeader && <h3 className="text-sm font-bold uppercase tracking-wider text-gray-500 mt-8 mb-4 border-b border-gray-200 dark:border-[#262626] pb-2">{dateLabel}</h3>}
              <div 
      ...
      <div className="h-20 shrink-0 border-b border-gray-200 dark:border-[#262626] px-8 flex items-center justify-between bg-white dark:bg-[#141414]">
        ...
        {viewMode === 'grid' && (
          <div className="flex items-center gap-6">
            <button onClick={handleToday} className="px-3 py-1.5 text-sm font-semibold text-gray-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-900 rounded-lg transition-colors border border-gray-200 dark:border-[#262626]">Today</button>
            <div className="flex items-center gap-4">
            ...
            {days.map((day, idx) => {
              ...
              return (
                <div 
                  ...
                  className={`bg-white dark:bg-[#18181b] rounded-xl border p-2 flex flex-col min-h-0 transition-colors cursor-pointer hover:border-accent/50 ${isToday ? 'border-accent shadow-sm ring-1 ring-accent/30' : 'border-gray-200 dark:border-[#262626]'}`}
                >
...
const EventModal: React.FC<{ isOpen: boolean, onClose: () => void, initialDate: string, ev: CalendarEvent | null, onSave: (e: CalendarEvent) => void, onDeleteRequest: (id: string) => void }> = ({ onClose, initialDate, ev, onSave, onDeleteRequest }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="relative w-full max-w-md bg-white dark:bg-[#18181b] border border-gray-200 dark:border-[#262626] rounded-xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full mt-1 px-3 py-2 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-[#262626] rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-accent [color-scheme:light] dark:[color-scheme:dark]" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase">Time <span className="opacity-50">(Optional)</span></label>
              <input type="time" value={time} onChange={e => setTime(e.target.value)} className="w-full mt-1 px-3 py-2 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-[#262626] rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-accent [color-scheme:light] dark:[color-scheme:dark]" />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="Add notes or location..." className="w-full mt-1 px-3 py-2 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-[#262626] rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-accent h-24 resize-none" />
          </div>
        ...
        <div className="p-6 border-t border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-[#1e1e1e] flex justify-end gap-3">
