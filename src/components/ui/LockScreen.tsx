...
export const LockScreen: React.FC = () => {
  ...
  const triggerErrorShake = () => {
    ...
    const handleStepperComplete = async (newWorkspaceName: string, newPassword: string) => {
    setIsLoading(true);
    setError(null);
    try {
      ...
      const { key, salt } = await saveVault(newPassword, initialData);
      await saveAppConfig({ workspaceName: newWorkspaceName });
      unlockVault(key, salt, initialData);
    } catch (err: any) {
      setError(err.message || "Cryptographic generation failed.");
      setIsLoading(false);
    }
  };
  ...
  if (step === 'welcome' || step === 'create') {
    return <OnboardingStepper onComplete={handleStepperComplete} />;
  }
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden flex items-center justify-center bg-[#050507]">
      ...
      <div className="relative z-10 w-full max-w-sm px-4">
        ...
        <div className="bg-[#050507]/40 backdrop-blur-2xl border border-white/10 rounded-xl shadow-2xl p-10 w-full flex flex-col items-center text-center">
