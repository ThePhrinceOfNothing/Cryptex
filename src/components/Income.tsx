...
export const Income: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 h-full bg-white flex flex-col">
      <div className="flex justify-between items-center mb-8 shrink-0">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Track your encrypted income and expenses.</p>
        </div>
        <motion.button
      ...
      <div className="grid grid-cols-3 gap-6 mb-8 shrink-0">
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total In</span>
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total Out</span>
          </div>
      ...
      {/* Chart Section */}
      {chartData.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm mb-8 shrink-0 h-64">
      ...
      {isAdding && (
        <form onSubmit={handleAdd} className="mb-8 p-5 bg-gray-50 border border-gray-200 rounded-xl flex items-end gap-4 shrink-0">
          <div className="flex-1">
            <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-1">Category / Description</label>
            <input
              type="text"
              value={category}
          ...
          <div className="w-32">
            <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-1">Amount</label>
            <input
              type="number"
              step="0.01"
              value={amount}
      ...
      {/* Transactions Table */}
      <div className="flex-1 overflow-hidden flex flex-col bg-white border border-gray-200 rounded-xl shadow-sm relative">
        <div className="grid grid-cols-4 gap-4 px-6 py-3 bg-gray-50 border-b border-gray-200 shrink-0">
          <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Date</div>
          <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider col-span-2">Category</div>
          ...
          {transactions.length === 0 ? (
            ...
            <AnimatePresence initial={false}>
              {transactions.map(tx => (
                ...
                >
                  <div className="grid grid-cols-4 gap-4 px-6 py-4 border-b border-gray-100 hover:bg-gray-50 transition-colors items-center text-sm">
                    <div className="text-gray-500">{new Date(tx.date).toLocaleDateString()}</div>
                    <div className="col-span-2 text-gray-900 font-medium">{tx.category}</div>
                    <div className={`text-right font-medium ${tx.type === 'in' ? 'text-emerald-600' : 'text-gray-900'}`}>
